import React from "react";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import NewImage from "../assets/NewImage.jpg";

function ForgotPassword() {
  const navigate = useNavigate();

  const handleReset = (e) => {
    e.preventDefault();
    // Tu peux ici envoyer l'email à ton backend ou API
    alert("Un lien de réinitialisation a été envoyé à votre adresse email.");
  };

  const inputClass =
    "w-full border border-gray-300 focus:outline-none focus:border-purple-500 py-2 px-3 rounded-md shadow-sm focus:shadow-md transition duration-300 text-sm";

  return (
    <div className="w-full h-screen flex items-center justify-center bg-gradient-to-r from-purple-400 via-purple-500 to-purple-700">
      <div className="w-full max-w-5xl h-[90vh] flex bg-white shadow-2xl rounded-xl overflow-hidden">
        {/* Image à gauche */}
        <div className="w-1/2 hidden md:flex items-center justify-center">
          <motion.img
            initial={{ opacity: 0, x: -100 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -100 }}
            transition={{ duration: 0.6, ease: "easeInOut" }}
            className="h-full w-full object-cover object-center"
            src={NewImage}
            alt="Illustration principale"
          />
        </div>

        {/* Formulaire à droite */}
        <div className="w-full md:w-1/2 flex flex-col items-center justify-center px-6 py-10">
          <h1 className="text-2xl text-gray-700 mb-4">Mot de passe oublié ?</h1>
          <p className="text-xs text-gray-600 mb-6 text-center">
            Entrez votre adresse e-mail et nous vous enverrons un lien
            <br />
            Pour réinitialiser votre mot de passe.
          </p>

          <form className="w-full max-w-xs" onSubmit={handleReset}>
            <div className="mb-4">
              <label className="text-gray-600 text-xs">Adresse email</label>
              <input
                type="email"
                required
                className={inputClass}
                placeholder="exemple@email.com"
              />
            </div>

            <button
              type="submit"
              className="w-full bg-purple-500 text-white text-xs py-2 rounded-full hover:bg-purple-600 transition duration-300 ease-in-out transform hover:scale-105"
            >
              Envoyer le lien
            </button>

            <div className="mt-4 text-center">
              <span
                className="text-purple-500 text-xs cursor-pointer hover:underline"
                onClick={() => navigate("/login")}
              >
                Retour à la connexion
              </span>
            </div>
          </form>
        </div>
      </div>
    </div>
  );
}

export default ForgotPassword;
